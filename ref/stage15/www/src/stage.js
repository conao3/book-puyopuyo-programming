class Stage {
  ...
  static initialize() {
    ...
    Stage.nextElement.style.left =
      (Config.puyoImageWidth * (Config.stageCols - 1)) / 2 - borderWidth + 'px';
    Stage.nextElement.style.top =
      Config.puyoImageHeight * 0.1 - borderWidth + 'px';
  ...
  static showZenkeshi() {
    ...
    Stage.zenkeshiImage.style.transition =
      'top ' + Config.zenkeshiDuration + 'ms linear';
    Stage.zenkeshiImage.style.top =
      (Config.puyoImageHeight * Config.stageRows) / 3 + 'px';
  }

  // 全消しの画像を画面上から消す
  static hideZenkeshi() {
    Stage.zenkeshiImage.style.transition =
      'opacity ' + Config.zenkeshiDuration + 'ms linear';
    Stage.zenkeshiImage.style.opacity = '0';
  }
}
